...
class SaveWysiwygContentAsUnformattedFilter extends AbstractFilter {
    public function filter($value, $postId, $field) {
        if (empty($GLOBALS['acf_layout_editor_content'])) {
            return $value;
        }

        if ($field['type'] === 'repeater' || $field['type'] === 'group' || $field['type'] === 'offbeat_components') {
            ...
            if(($field['type'] === 'repeater' || $field['type'] === 'group') && empty($field['sub_fields'])) {
                return $value;
            }
            ...
            foreach( array_keys($value) as $i ) {
                ...
                foreach( array_keys($sub_fields) as $j ) {
                    ...
                    } elseif( is_array($value[ $i ]) && array_key_exists($sub_field['key'], $value[ $i ]) ) {
